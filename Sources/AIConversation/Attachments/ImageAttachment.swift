...
/// Encodes a picked photo into an ``OutgoingAttachment`` ready for `POST /messages`.
///
/// Downsamples to a 1024 px long edge and JPEG-compresses at 0.7 so a multi-megapixel camera
/// shot stays under the API's 5 MiB (6,990,508 base64-char) cap. An optional tighter decoded-byte
/// budget (e.g. upload-prompt markers) gets one retry at a 512 px long edge before failing.
...
enum ImageAttachment {
    ...
    struct Options: Sendable, Equatable {
        /// Wire filename (composer sends none).
        var filename: String?
        ...
        /// Chat-message image with an optional tighter decoded-byte budget (upload-prompt markers).
